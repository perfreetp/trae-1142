...
export default function Training() {
  ...
  const {
    selectedDate, setSelectedDate, getPlansByDate,
    getCompletionRate, getRecordSubmissionRate, getCompletionDetails,
    getRecordByDate, addPlan,
  } = useTrainingStore();
  ...
  const checkInRate = selectedPlan ? getCompletionRate(selectedPlan.id) : 0;
  const submitRate = selectedPlan ? getRecordSubmissionRate(selectedPlan.id) : 0;
        ...
        <div className="animate-slide-up">
          ...
          <div className="flex gap-1.5">
            {weekDates.map((date, i) => {
              ...
              return (
                <button key={date} onClick={() => setSelectedDate(date)} className={cn('flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl transition-all', isSelected ? 'bg-gradient-brand shadow-glow' : 'bg-brand-card hover:bg-brand-card-hover')}>
                  ...
                  {plans.length > 0 && (<div className={cn('w-1.5 h-1.5 rounded-full', isSelected ? 'bg-white' : 'bg-brand-green')} />)}
                </button>
              );
            })}
          </div>
          ...
          <div className="animate-slide-up space-y-4" style={{ animationDelay: '0.05s' }}>
            <div className="brand-card">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={cn('text-xs px-2 py-0.5 rounded-full font-medium', typeColors[selectedPlan.type])}>{typeLabels[selectedPlan.type]}</span>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 bg-brand-deeper rounded-xl">
                  ...
                  <div><p className="text-xs text-brand-gray mb-0.5">热身</p><p className="text-sm text-white">{selectedPlan.warmup}</p></div>
                </div>
                <div className="flex items-start gap-3 p-3 bg-brand-deeper rounded-xl">
                  ...
                  <div><p className="text-xs text-brand-gray mb-0.5">主训练</p><p className="text-sm text-white">{selectedPlan.mainSession}</p></div>
                </div>
                <div className="flex items-start gap-3 p-3 bg-brand-deeper rounded-xl">
                  ...
                  <div><p className="text-xs text-brand-gray mb-0.5">放松</p><p className="text-sm text-white">{selectedPlan.cooldown}</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="brand-card">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-2"><Timer className="w-4 h-4 text-brand-cyan" /><div><p className="text-[10px] text-brand-gray">目标配速</p><PaceDisplay pace={selectedPlan.targetPace} /></div></div>
                <div className="flex items-center gap-2"><Dumbbell className="w-4 h-4 text-brand-orange" /><div><p className="text-[10px] text-brand-gray">心率区间</p><span className="text-sm font-semibold text-brand-orange">{selectedPlan.heartRateZone}</span></div></div>
                <div className="flex items-center gap-2"><CalendarDays className="w-4 h-4 text-brand-green" /><div><p className="text-[10px] text-brand-gray">目标距离</p><span className="text-sm font-semibold text-brand-green">{selectedPlan.targetDistance}km</span></div></div>
                <div className="flex items-center gap-2"><Users className="w-4 h-4 text-purple-400" /><div><p className="text-[10px] text-brand-gray">训练对象</p><span className="text-sm font-semibold text-purple-400">{selectedPlan.targetGroup}</span></div></div>
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {executionSteps.map((step, i) => (
                    <div key={i} className={cn('flex items-center gap-3 p-3 rounded-xl transition-all', step.completed ? 'bg-brand-green/5' : 'bg-brand-deeper')}>
                      ...
                      {!step.completed && (
                        <Link to={step.path} className="text-xs text-brand-cyan hover:text-brand-cyan/80 shrink-0">去完成</Link>
                      )}
                      {step.completed && step.label === '训练记录' && (
                        <Link to="/training/record" className="text-xs text-brand-gray hover:text-white shrink-0">修改</Link>
                      )}
                      {step.completed && step.label === '疲劳自评' && (
                        <Link to="/training/fatigue" className="text-xs text-brand-gray hover:text-white shrink-0">修改</Link>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {role === 'coach' && (
              <div className="brand-card">
                <h4 className="text-sm font-medium text-white mb-3">训练复盘</h4>
                      ...
                      <div key={detail.userId} className="bg-brand-deeper rounded-xl overflow-hidden">
                        <button onClick={() => setExpandedMember(isExpanded ? null : detail.userId)} className="w-full flex items-center gap-3 p-3">
                          ...
                          <div className="flex-1 text-left min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {detail.fatigue >= 7 && (
                                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-red/20 text-brand-red font-medium">疲劳</span>
                              )}
                            </div>
                            {detail.completed ? (
                        ...
                        {isExpanded && detail.completed && (
                          <div className="px-3 pb-3 pt-1 border-t border-brand-border grid grid-cols-3 gap-2 animate-slide-up">
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">距离</p>
                              <p className="text-sm font-semibold text-white">{detail.distance}km</p>
                            </div>
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">用时</p>
                              <p className="text-sm font-semibold text-white">{detail.duration}min</p>
                            </div>
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">配速</p>
                              <p className="text-sm font-semibold text-brand-cyan">{formatPaceSec(detail.pace)}/km</p>
                            </div>
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">心率</p>
                              <p className="text-sm font-semibold text-white">{detail.avgHR}bpm</p>
                            </div>
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">疲劳</p>
                              <p className={cn('text-sm font-semibold', detail.fatigue >= 7 ? 'text-brand-red' : detail.fatigue >= 4 ? 'text-brand-orange' : 'text-brand-green')}>RPE {detail.fatigue}</p>
                            </div>
                            <div className="text-center p-2 bg-brand-card rounded-lg">
                              <p className="text-[10px] text-brand-gray">打卡</p>
                              <p className={cn('text-sm font-semibold', detail.checkedIn ? 'text-brand-green' : 'text-brand-gray')}>{detail.checkedIn ? '已打卡' : '未打卡'}</p>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            {role === 'member' && (
              <div className="flex gap-3">
                <Link to="/training/record" className="brand-btn flex-1 flex items-center justify-center gap-2">
                  {existingRecord?.completed ? '修改记录' : '记录训练'}
                  <ArrowRight className="w-4 h-4" />
                </Link>
                {existingRecord?.completed && (
                  <Link to="/training/checkin" className={cn('flex-1 flex items-center justify-center gap-2 rounded-xl py-3 font-semibold transition-all', existingRecord.checkedIn ? 'bg-brand-green/20 text-brand-green' : 'bg-gradient-brand text-white shadow-glow')}>
