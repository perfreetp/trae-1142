...
export default function TrainingRecord() {
  ...
  useEffect(() => {
    if (existingRecord) {
      setDistance(existingRecord.distance > 0 ? String(existingRecord.distance) : '');
      setDuration(existingRecord.duration > 0 ? String(existingRecord.duration) : '');
      setAvgHR(existingRecord.avgHR > 0 ? String(existingRecord.avgHR) : '');
      setZones(existingRecord.heartRateZones);
      setFatigue(existingRecord.fatigue || 5);
  ...
  const handleSave = () => {
    ...
    const recordData = {
      ...
      fatigue,
      note,
      checkedIn: existingRecord?.checkedIn ?? false,
      completed: true,
    };
  ...
  const fatigueColor = fatigue <= 3 ? 'text-brand-green' : fatigue <= 6 ? 'text-brand-orange' : 'text-brand-red';

  return (
    <div className="min-h-screen bg-brand-dark">
      <Header title={existingRecord ? '修改训练记录' : '训练记录'} showBack />
      ...
      <div className="page-content space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="flex items-center gap-1.5 text-xs text-brand-gray mb-1.5">
                <Ruler className="w-3.5 h-3.5" />距离 (km)
              </label>
              <input type="number" value={distance} onChange={(e) => setDistance(e.target.value)} placeholder="0.0" step="0.1" min="0" className="brand-input w-full" />
            </div>
            <div>
              <label className="flex items-center gap-1.5 text-xs text-brand-gray mb-1.5">
                <Clock className="w-3.5 h-3.5" />用时 (分钟)
              </label>
              <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="0" min="0" className="brand-input w-full" />
            </div>
          </div>
          {calculatedPace > 0 && (
        ...
        <div className="brand-card animate-slide-up" style={{ animationDelay: '0.05s' }}>
          ...
          <div className="mb-4">
            <label className="flex items-center gap-1.5 text-xs text-brand-gray mb-1.5">
              <Heart className="w-3.5 h-3.5 text-brand-red" />平均心率 (bpm)
            </label>
            <input type="number" value={avgHR} onChange={(e) => setAvgHR(e.target.value)} placeholder="0" min="0" max="220" className="brand-input w-full" />
          </div>
          <HeartRateZones zones={zones} />
          <div className="grid grid-cols-5 gap-2 mt-3">
            {(['z1', 'z2', 'z3', 'z4', 'z5'] as const).map((key) => (
              <div key={key} className="text-center">
                <input type="number" value={zones[key]} onChange={(e) => handleZoneChange(key, Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))} className="brand-input w-full text-center text-xs py-1.5 px-1" min="0" max="100" />
        ...
        <div className="brand-card animate-slide-up" style={{ animationDelay: '0.1s' }}>
          ...
          <input type="range" min="1" max="10" value={fatigue} onChange={(e) => setFatigue(parseInt(e.target.value))} className="w-full h-2 rounded-full appearance-none cursor-pointer" style={{ background: `linear-gradient(to right, #10B981 0%, #F97316 60%, #EF4444 100%)` }} />
          <div className="flex justify-between text-[10px] text-brand-gray mt-1">
            <span>非常轻松</span><span>适中</span><span>极限</span>
          </div>
        </div>

        <div className="brand-card animate-slide-up" style={{ animationDelay: '0.15s' }}>
          <label className="flex items-center gap-1.5 text-xs text-brand-gray mb-1.5">
            <StickyNote className="w-3.5 h-3.5" />训练笔记
          </label>
          <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="记录今天训练的感受..." rows={3} className="brand-input w-full resize-none" />
        </div>

        <button onClick={handleSave} disabled={!distNum || !durNum} className="brand-btn w-full disabled:opacity-40 disabled:cursor-not-allowed">
          {existingRecord ? '保存修改' : '保存记录'}
        </button>
      </div>
    </div>
  );
