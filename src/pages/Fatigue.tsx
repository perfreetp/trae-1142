    ...
    <div className="min-h-screen bg-brand-dark">
      ...
      <div className="page-content space-y-6">
        <div className="flex flex-col items-center py-6 animate-slide-up">
          <div className={cn(
            'w-28 h-28 rounded-full flex items-center justify-center border-2 transition-all duration-300',
            getFatigueBg(rpe),
          )}>
            <span className={cn('font-display text-5xl font-bold transition-colors duration-300', getFatigueColor(rpe))}>
              {rpe}
            </span>
          </div>
          <p className={cn(
            'mt-3 font-display text-lg font-semibold transition-colors duration-300',
            getFatigueColor(rpe),
          )}>
            {rpeDescriptions[rpe]}
          </p>
          ...
          <div className="relative">
            <div className="h-3 rounded-full overflow-hidden mb-2">
              <div
                ...
                style={{
                  background: 'linear-gradient(to right, #10B981, #22D3EE, #F97316, #EF4444)',
                }}
              />
            </div>
            <input
              ...
              className="w-full absolute top-0 left-0 opacity-0 cursor-pointer h-3"
            />
        ...
        <div className="brand-card animate-slide-up" style={{ animationDelay: '0.15s' }}>
          ...
          <textarea
            ...
            placeholder="记录身体感受、不适部位等..."
            rows={3}
            className="brand-input w-full resize-none"
          />
        </div>

        <button onClick={handleSubmit} className="brand-btn w-full">
          提交评估
        </button>
      </div>
    </div>
